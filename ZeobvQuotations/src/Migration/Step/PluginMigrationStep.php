...
namespace Zeobv\Quotations\Migration\Step;
...
abstract class PluginMigrationStep extends MigrationStep
{
    ...
    protected function createCustomField(Connection $connection, array $customField, ?string $customFieldSetName = null): void
    {
        if ($customFieldSetName) {
            $customField['set_id'] = $this->getCustomFieldSetIdByName($customFieldSetName);
        }
    ...
    protected function getCustomFieldSetIdByName(Connection $connection, string $customFieldSetName)
    {
        return $connection->executeQuery("SELECT HEX(id) FROM custom_field_set WHERE name = :name", [
    ...
    {
        if (!key_exists('id', $entityData)) {
            $entityData['id'] = Uuid::randomBytes();
        }

        if (!key_exists('created_at', $entityData)) {
