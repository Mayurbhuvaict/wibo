 ...
 */
class HisenseStockImportController extends AbstractController
{
    ...
    /**
     * @Route("/api/zeostock/hisensesupplier", name="api.action.zeo.hisense.import", methods={"GET"})
     */
    public function hisenseStockImport(Context $context): JsonResponse
    {
        $username = 'SCHOUWWIT';
        $password = '4A2BbE99d';
                    ...
                    if (empty($dataExist)) {
                        switch ($item->ATPCode) {
                            ...
                            case '2':
                                ...
                                break;
                            case '9':
                                $message = 'error code '.$item->ATPCode.':'.$item->EAN.' | '.
                                    $item->ArticleCode. PHP_EOL;
                                $stock = 0;
                                $date = '';
                                $this->updateProduct($product, $stock, $date, $context);
                                break;
    ...
    public function getAllProduct(Context $context): array
    {
        ...
        $criteria->addFilter(new EqualsFilter('customFields.migration_attribute_19_inkooppartij_197', 'option_1701'));
        $criteria->addFilter(new EqualsFilter('manufacturer.name', 'hisense'));
    ...
    public function updateProduct($product, $stock, $date, $context)
    {
        $data = [
            ...
            'stock'         => $stock,
            'customFields' => [
                'migration_attribute_19_inv_leverdatum_262' => $date
            ],
        ];
    ...
    public function checkSupplierTable($product, $apiData, Context $context)
    {
        ...
        $criteria->addFilter(new EqualsFilter('hisenseApiRecord', json_encode($apiData)));
    ...
    public function updateSupplierTable($product, $apiData, $dataExist, Context $context)
    {
        $data = [
            ...
            'hisenseApiRecord'  => json_encode($apiData),
        ];
