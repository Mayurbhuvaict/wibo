 ...
 */
class AskoStockImportController extends AbstractController
{
    ...
    /**
     * @Route("/api/zeostock/askosupplier", name="api.action.zeo.asko.import", methods={"GET"})
     */
    public function askoStockImport(Context $context): JsonResponse
    {
        $username = 'SCHOUWWIT';
        $password = '4A2BbE99d';
    ...
    public function getAllProduct(Context $context): array
    {
        ...
        $criteria->addFilter(new EqualsFilter('manufacturer.name', 'asko'));
    ...
    public function checkSupplierTable($product, $apiData, Context $context)
    {
        ...
        $criteria->addFilter(new EqualsFilter('askoApiRecord', json_encode($apiData)));
    ...
    public function updateSupplierTable($product, $apiData, $dataExist, Context $context)
    {
        $data = [
            ...
            'askoApiRecord'  => json_encode($apiData),
        ];
